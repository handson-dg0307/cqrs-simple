...
설명:
   CQRS 패턴 실습을 위한 Azure 리소스를 생성하고 애플리케이션을 AKS에 배포합니다.
...
check_prerequisites() {
   ...
   if ! command -v kubectl &> /dev/null; then
       log "Error: kubectl가 설치되어 있지 않습니다."
       exit 1
   fi
}
...
setup_environment() {
   ...
   AKS_NAME="aks-digitalgarage-${POSTFIX}"
   ...
   # Event Hub 설정 - 단일 EventHub 사용으로 변경
...
setup_acr_permission() {
   ...
   # AKS가 ACR에서 이미지를 가져올 수 있도록 권한 부여
   az aks update \
       --name $AKS_NAME \
       --resource-group $RESOURCE_GROUP \
       --attach-acr $ACR_NAME \
       --output none
   check_error "ACR 권한 설정 실패"
}
...
EOF
   ...
   # Docker 이미지 빌드
   docker build -f Dockerfile-command -t ${ACR_NAME}.azurecr.io/telecom-${USERID}/cqrs-command:v1 .
   check_error "Command 서비스 이미지 빌드 실패"

   docker build -f Dockerfile-query -t ${ACR_NAME}.azurecr.io/telecom-${USERID}/cqrs-query:v1 .
...
push_images() {
   ...
   # 이미지 푸시
   docker push ${ACR_NAME}.azurecr.io/telecom-${USERID}/cqrs-command:v1
   check_error "Command 서비스 이미지 푸시 실패"

   docker push ${ACR_NAME}.azurecr.io/telecom-${USERID}/cqrs-query:v1
   check_error "Query 서비스 이미지 푸시 실패"

   # 이미지 태그 확인
   az acr repository list --name $ACR_NAME --output table
   az acr repository show-tags --name $ACR_NAME --repository telecom-${USERID}/cqrs-command --output table
   az acr repository show-tags --name $ACR_NAME --repository telecom-${USERID}/cqrs-query --output table
}
...
setup_db_secrets() {
   ...
   kubectl delete secret $DB_SECRET_NAME --namespace $DB_NAMESPACE 2>/dev/null || true
   kubectl delete secret $DB_SECRET_NAME --namespace $APP_NAMESPACE 2>/dev/null || true
...
setup_event_hub() {
   ...
   if [ -z "$EXISTING_NS" ]; then
       log "Event Hub 네임스페이스 생성 중... (약 2-3분 소요)"
   ...
   kubectl create secret generic eventhub-secret \
       --namespace $APP_NAMESPACE \
       --from-literal=connection-string="$EVENT_HUB_CONNECTION_STRING" \
       --from-literal=hub-name="$EVENT_HUB_NAME" \
...
setup_storage() {
    ...
    kubectl create secret generic storage-secret \
        ...
        --from-literal=container="$BLOB_CONTAINER" \
        2>/dev/null || true
 ...
 template:
   ...
   spec:
     ...
     - name: command-service
       image: ${ACR_NAME}.azurecr.io/telecom-${USERID}/cqrs-command:v1
...
spec:
 ...
 template:
   ...
   spec:
     containers:
     - name: query-service
       image: ${ACR_NAME}.azurecr.io/telecom-${USERID}/cqrs-query:v1
       imagePullPolicy: Always
       ports:
       - containerPort: 8080
       envFrom:
...
print_completion_message() {
   ...
   log "Query Service URL: http://$QUERY_IP:8081/swagger-ui.html"
}
